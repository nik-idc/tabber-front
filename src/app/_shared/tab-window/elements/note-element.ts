...
import { GuitarNote } from "src/app/_models/tab/guitar-note";
...
export class NoteElement {
	...
	constructor (tabWindow: TabWindow, chordElement: ChordElement, note: GuitarNote) {
	...
	calc(): void {
		...
		this.rect.y = this.chordElement.rect.y - dim.noteMinSize / 2 + dim.noteMinSize * (this.note.strNum - 1);

		this.textCoords.x = this.chordElement.rect.x + this.chordElement.rect.width / 2;
