...
// import { BarsLineElement } from "./elements/bars-line-element";
import { NoteElement } from "./elements/note-element";
import { BarElement } from "./elements/bar-element";
import { Point } from "./shapes/point";
import { Bar } from "src/app/_models/tab/bar";
import { NoteDuration } from "src/app/_models/tab/note-duration";
import { Note } from "src/app/_models/tab/note";
...
export class TabWindow {
	...
	private createBarElements(): void {
		...
		for (let barId = 0; barId < this.tab.bars.length; barId++) {
			let showMeasure;
			if (barId == 0 || this.tab.bars[barId - 1].measure != this.tab.bars[barId].measure) {
				showMeasure = true;
			} else {
				showMeasure = false;
			}
	...
	private splitIntoLines(): void {
		...
		// Split into lines
		for (let barId = 0; barId < this.barElements.length; barId += this.dim.barsPerLine) {
			...
			let line = Array.from(Array(lineEndId - lineStartId).keys(), num => num + lineStartId);
	...
	moveTabSelectedNoteDown(): void {
		...
		this.selectedNoteElement = this.selectedNoteElement.chordElement.noteElements[newStrNum - 1];
	}
	...
	moveTabSelectedNoteLeft(): void {
		...
		if (chordElementId > 0 && barElement.chordElements.length > 1) {
			...
			this.selectedNoteElement = barElement.chordElements[newChordElementId].noteElements[strNum - 1];
			...
			let newBarLastChordId = this.barElements[newBarElementId].chordElements.length - 1;
			this.selectedNoteElement = this.barElements[newBarElementId].chordElements[newBarLastChordId].noteElements[strNum - 1];
		}
	}
	...
	moveTabSelectedNoteRight(): void {
		...
		if (chordElementId < barElement.chordElements.length - 1) {
			...
			let newBarElementId = barElementId + 1;
			this.selectedNoteElement = this.barElements[newBarElementId].chordElements[0].noteElements[strNum - 1];
		}
	}
